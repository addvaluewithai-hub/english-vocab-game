...
const FILTERS: readonly { value: BankFilter; label: string }[] = [
  { value: 'ALL', label: 'All' },
  { value: 'LEARNING', label: 'Learning / due' },
        ...
        <Surface style={{ padding: spacing.md, gap: spacing.sm }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: spacing.md, alignItems: 'flex-start' }}>
            <View style={{ flex: 1, gap: spacing.xs }}>
              <Text selectable numberOfLines={2} style={{ color: colors.ink, fontSize: 22, fontWeight: '800' }}>{item.term}</Text>
          ...
          <Text selectable style={{ color: colors.inkMuted, fontSize: typography.small }}>{item.reviewCount} reviews{item.sourceTitle ? ` · ${item.sourceTitle}` : ''}</Text>
        </Surface>
      </Pressable>
    </Link>
  );
...
export function BankScreen() {
  ...
  if (!pair) {
    return <EmptyState title="Choose your languages" body="Set a learning language and an explanation language before adding vocabulary." action={<Link href="/settings" asChild><ActionButton label="Open settings" /></Link>} />;
  }
        ...
        ListHeaderComponent={
          <View style={{ gap: spacing.md, marginBottom: spacing.md }}>
            <View style={{ gap: spacing.xs }}>
              <Text selectable style={{ color: colors.ink, fontSize: typography.title, fontWeight: '800' }}>{pair.targetLanguageName}</Text>
              <Text selectable style={{ color: colors.inkMuted, fontSize: typography.label }}>Meanings in {pair.referenceLanguageName}</Text>
            </View>
            <TextInput
            ...
            <View accessibilityRole="tablist" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>
              {FILTERS.map((option) => (
                <Pressable
                  ...
                  style={{ paddingHorizontal: 14, paddingVertical: 9, borderRadius: radius.pill, backgroundColor: filter === option.value ? colors.accent : colors.surfaceMuted }}
                >
                  <Text style={{ color: filter === option.value ? colors.surface : colors.inkMuted, fontWeight: '700' }}>{option.label}</Text>
                </Pressable>
              ))}
            </View>
            <View style={{ flexDirection: 'row', gap: spacing.sm }}>
              <View style={{ flex: 1 }}><Link href="/add" asChild><ActionButton label="Add vocabulary" /></Link></View>
              <View style={{ flex: 1 }}><Link href="/collections" asChild><ActionButton label="Collections" /></Link></View>
            </View>
            <Link href="/import-staging" asChild><ActionButton label="Review import candidates" /></Link>
          </View>
        }
        ListEmptyComponent={<EmptyState title={search || filter !== 'ALL' ? 'No matches' : 'Your bank is empty'} body={search || filter !== 'ALL' ? 'Try a different search or filter.' : 'Add your first word or phrase to start building your personal bank.'} />}
      />
    </View>
  );
}
