...
export function StudyScreen() {
  ...
  const [snapshot, setSnapshot] = useState<StudySessionSnapshot | null>(() =>
    getActiveStudySession()?.snapshot ?? null,
  );
  ...
  if (error) {
    return (
      <EmptyState
        ...
        body={error}
        action={
          <ActionButton label="Try again" onPress={() => void restartSession()} />
        }
      />
    );
  }
  ...
  if (!snapshot || totalCards === null) {
    return (
      ...
      >
        <ActivityIndicator accessibilityLabel="Loading study session" />
      </View>
    );
  }
  ...
  if (totalCards === 0) {
    return (
      <EmptyState
        ...
        body="Add your first word or phrase, then come back here to practice it."
        action={
          <Link href="/add" asChild>
            <ActionButton label="Add vocabulary" />
          </Link>
        }
      />
    );
  ...
  if (snapshot.completed && snapshot.summary.reviewed === 0) {
    return (
      <EmptyState
        ...
        body="You are caught up. Come back when your next review is due."
        action={
          <Link href="/bank" asChild>
            <ActionButton label="Open vocabulary bank" />
          </Link>
        }
      />
    );
  ...
  if (snapshot.completed) {
    return (
      ...
      >
        ...
        >
          {snapshot.summary.reviewed} reviews · {snapshot.summary.knew} knew ·{' '}
          {snapshot.summary.forgot} forgot · {snapshot.summary.retries} retries
        </Text>
        <View style={{ gap: spacing.sm }}>
          <ActionButton
            label="Check for due cards"
            onPress={() => {
          ...
          />
          <Link href="/bank" asChild>
            <ActionButton label="Vocabulary bank" />
          </Link>
        </View>
      </ScrollView>
    );
    ...
    >
      <View style={{ gap: spacing.sm }}>
        ...
        >
          ...
          >
            {snapshot.reviewedCount + 1} / {snapshot.plannedTotal}
          </Text>
          {current.isRetry ? (
            <Text
              selectable
      ...
      <View style={{ flex: 1, justifyContent: 'center', minHeight: 420 }}>
        <SwipeGradeCard
          ...
          onReveal={() => {
            const now = Date.now();
            const startedAt = cardStartedAt.current ?? now;
            recallMs.current = Math.max(0, now - startedAt);
            setRevealed(true);
          }}
      ...
      {!revealed ? (
        ...
        >
          Think first. Reveal before grading.
        </Text>
      ) : null}
    </ScrollView>
  );
